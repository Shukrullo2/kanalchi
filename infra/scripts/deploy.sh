...
#   /opt/kanalchi/infra/scripts/deploy.sh [<commit-sha>]
#
# The Actions key is installed with a forced command pointing here, so the SHA arrives in
# SSH_ORIGINAL_COMMAND and the key cannot run anything else. The repo is public, so the server
# fetches over HTTPS without credentials. infra/.env is untracked and never touched.
#
...
main() {
  ...
  want="${want##* }"                       # accept "deploy <sha>" or "<sha>"
  ...
  local dc=(docker compose -f infra/compose.yml --env-file infra/.env)
  "${dc[@]}" build --pull
  ...
  for i in $(seq 1 60); do
    if [[ "$("${dc[@]}" ps api --format '{{.Health}}' 2>/dev/null)" == "healthy" ]]; then
      ...
      docker image prune -f --filter "label=com.docker.compose.project=kanalchi" >/dev/null || true
